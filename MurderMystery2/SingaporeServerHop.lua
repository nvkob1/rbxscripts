...
local function GetLowestPingSingaporeServer(servers)
  local lowestPing = math.huge
  local bestServer = nil
  local bestIndex = nil
  
  for i, server in pairs(servers) do
      if server.ping and server.ping < lowestPing and server.location and 
         (string.find(string.lower(server.location.country), "singapore") or 
          string.find(string.lower(server.location.country), "sg")) then
...
local function StartTeleport()
  ...
  for i = 0, 99 do
      ...
      local bestServer, bestIndex = GetLowestPingSingaporeServer(JSONData.data)
      if bestServer then
          ...
          -- No Singapore servers found, get next page
          if JSONData.nextPageCursor then
              EncodeToFile(NextCursor(JSONData.nextPageCursor))
          else
              warn("No Singapore servers found")
              break
          end
      end
  end
