...
local function OnMapLoad(Map)
    ...
    while task.wait(CHECK_DELAY) and Check("InGame") do
        ...
        if not ExitRegion then
            -- FIXED: Remove invalid buttons and iterate backwards for safe removal
            ...
            for i, Button in pairs(Buttons) do
                ...
                if ButtonHitbox then
                    ...
                    if TouchFound and GuiFound then
                        ...
                        -- FIXED: Teleport to button + bypass button anti-cheat with proper unanchoring
                        ...
                        HumanoidRootPart.Anchored = false -- FIXED: Always unanchor after button interaction
                        
                        -- FIXED: Check if button was pressed and remove it from list
                        task.wait(0.1)
                        if not Button:FindFirstChild("TouchInterest", true) or not Button:FindFirstChildWhichIsA("BillboardGui", true) then
                            table.remove(Buttons, i)
                        end
                    end
                end
    ...
    Humanoid:ChangeState(Enum.HumanoidStateType.Jumping)
    -- FIXED: Ensure character is unanchored before proceeding
